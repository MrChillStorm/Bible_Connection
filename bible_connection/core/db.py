...
ROOT = Path(__file__).resolve().parents[2]  # the repo, when running from a clone
...
# deliberately outside the app's own folder, so personal state isn't
# tied to wherever that folder happens to live (or to an installed
# package, which can't be written to). Two OS accounts sharing one copy
# of the app get independent reading histories, and moving,
# redownloading or reinstalling the app doesn't strand anyone's
# progress. appauthor=False skips Windows' extra publisher-named
# subfolder, which this project has no use for. BIBLE_CONNECTION_HOME
# points the app at another folder instead, e.g. to try something out
# without touching your own reading history.
_HOME_OVERRIDE = os.environ.get("BIBLE_CONNECTION_HOME")
DATA_DIR = Path(_HOME_OVERRIDE) if _HOME_OVERRIDE else Path(user_data_dir("Bible Connection", appauthor=False))

# The content database, extracted from the shipped bible.db.zip on first
# launch (see db_bootstrap.py), and personal reading state, side by side.
DB_PATH = DATA_DIR / "bible.db"
USER_DB_PATH = DATA_DIR / "user_state.db"
...
def get_connection() -> sqlite3.Connection:
    ...
    unchanged by normal use of the app, and shipped zipped in the
    package as data/bible.db.zip."""
...
def get_user_connection() -> sqlite3.Connection:
    ...
    the app's folder -- deliberately a separate file from the content
    database too, so the content database can be replaced by an update
    without touching anyone's reading history."""
    _migrate_user_db_location()
    USER_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(USER_DB_PATH)
    conn.row_factory = sqlite3.Row
    # the tables exist before anyone reads them -- on a first launch the
    # saved text size is looked up before the window (and
    # init_user_schema) exists
    conn.executescript(USER_SCHEMA_PATH.read_text())
    return conn
...
def _migrate_user_db_location() -> None:
    ...
    the checks below would wrongly skip the move. Skipped entirely under
    BIBLE_CONNECTION_HOME, which must never pull in the real history."""
    if _HOME_OVERRIDE:
        return
